RSpec.describe "Ruby's Hash structure" do
  ...
  it "has a fetch method for accessing elements which can return the result of a block if the specified key does not exist" do
    hash = { :one => 1, :two => 2  }

    expect( hash.fetch(:three) { |el| "#{el}" } ).to eq( 'three' ) 
  end
